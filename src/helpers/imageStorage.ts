...
type validFileExtensionType = 'png' | 'jpeg' | 'jpg'
type validMimeType = 'image/png' | 'image/jpef' | 'image/jpg'

const validFileExtension: validFileExtensionType[] = ['png', 'jpg', 'jpeg']
const validMimeTypes: validMimeType[] = ['image/png' , 'image/jpef' , 'image/jpg']
...
export const isFileExtensionSafe = async (fullFilePath: string): Promise<boolean> => {
    return (await FileType).fileTypeFromFile(fullFilePath).then((fileExtensionAndMimeType) => {
        ...
        const isFileTypeLegit = validFileExtension.includes(fileExtensionAndMimeType.ext as validFileExtensionType)
